 ...
 *
 */
public class CountBatches extends Configured implements Tool {

  protected enum Counter {
    records
  };

  private static final Logger LOG = Logger.getLogger(CountBatches.class);
  ...
  public static void main(String[] args) throws Exception {
    int res = ToolRunner.run(new Configuration(), new CountBatches(), args);
    System.exit(res);
  }

  static int printUsage() {
    System.out
        .println(CountBatches.class.getName()
            + " -i input -o output \n"
            + "Example usage: hadoop jar trec-kba.jar ilps.hadoop.CountBatches"
            + " -i kba/kba-stream-corpus-2012-cleansed-only-out-repacked"
            + " -o kba/kba-stream-corpus-2012-cleansed-only-out-repacked-batchcounts\n\n");
  ...
  public int run(String[] args) throws Exception {
    ...
    Job job = new Job(conf, "CountBatches");
    job.setJarByClass(CountBatches.class);
